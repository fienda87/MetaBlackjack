...
async function main() {
  ...
  const contractPath = path.join(process.cwd(), "artifacts/contracts/GBCToken.sol/GBCToken.json");
  const contractJson = JSON.parse(fs.readFileSync(contractPath, "utf8"));
  
  const GBCTokenFactory = new ethersLib.ContractFactory(
    contractJson.abi,
    contractJson.bytecode,
    deployer
  );
  
  const gbcToken = await GBCTokenFactory.deploy();
  ...
  console.log("💎 Initial supply:", ethersLib.formatEther(totalSupply), "GBC");
